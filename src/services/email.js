    ...
    html: `
      <div style="font-family:sans-serif;max-width:700px;margin:0 auto;background:#f8fafc;">
        <div style="background:#0f172a;padding:24px;border-radius:12px 12px 0 0;">
          ...
          <p style="color:#60a5fa;margin:4px 0 0;">Nueva inspección recibida</p>
        </div>
        ...
        <div style="padding:24px;background:white;">
          <div style="display:flex;gap:16px;margin-bottom:20px;flex-wrap:wrap;">
            ...
            <div style="background:#f1f5f9;padding:12px 16px;border-radius:8px;flex:1;min-width:150px;">
              <p style="margin:0;font-size:11px;color:#94a3b8;text-transform:uppercase;">Prioridad alta</p>
              <p style="margin:4px 0 0;font-weight:600;color:#d97706;font-size:22px;">${highSeverity.length}</p>
            </div>
          </div>

          <p style="margin:0 0 8px;color:#64748b;font-size:13px;"><strong>Dirección:</strong> ${inspection.address || 'No especificada'}</p>
          ...
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <thead>
              <tr style="background:#f1f5f9;">
                ...
                <th style="padding:8px;text-align:left;font-size:12px;color:#64748b;">RIESGO SI NO SE REPARA</th>
              </tr>
            </thead>
...
async function sendEstimateToClient(inspection) {
  ...
  await transporter.sendMail({
    ...
    subject: `Tu Estimado de Reparación — MacTor Maintenance`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#f8fafc;">
        <div style="background:#0f172a;padding:24px;border-radius:12px 12px 0 0;">
          ...
          <p style="color:#60a5fa;margin:4px 0 0;">Tu estimado de reparación</p>
        </div>

        <div style="padding:24px;background:white;">
          <p style="color:#0f172a;">Hola <strong>${inspection.clientName}</strong>,</p>
          <p style="color:#64748b;">Revisamos tu inspección en <strong>${inspection.address}</strong> y preparamos el siguiente estimado:</p>

          <table style="width:100%;border-collapse:collapse;margin:20px 0;">
            <thead>
              <tr style="background:#f1f5f9;">
                <th style="padding:10px;text-align:left;font-size:12px;color:#64748b;">TRABAJO</th>
                <th style="padding:10px;text-align:right;font-size:12px;color:#64748b;">COSTO</th>
              </tr>
            </thead>
          ...
          <p style="font-size:12px;color:#94a3b8;">Este estimado es válido por ${estimate.valid_days || 30} días.</p>
    ...
    html: `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;">
        <div style="background:#16a34a;padding:24px;border-radius:12px 12px 0 0;">
          <h1 style="color:white;margin:0;">✅ Estimado Aceptado</h1>
        </div>
        <div style="padding:24px;background:white;border-radius:0 0 12px 12px;">
          <p style="font-size:18px;font-weight:600;color:#0f172a;">Contactar al cliente para agendar visita:</p>
          <p><strong>Nombre:</strong> ${inspection.clientName}</p>
          <p><strong>Teléfono:</strong> ${inspection.clientPhone}</p>
          <p><strong>Email:</strong> ${inspection.clientEmail}</p>
          <p><strong>Dirección:</strong> ${inspection.address}</p>
          <p><strong>Total estimado:</strong> $${(inspection.approvedEstimate?.total || 0).toLocaleString()} CAD</p>
          <p><strong>Tipo:</strong> ${inspection.propertyType === 'commercial' ? 'Comercial' : 'Residencial'}</p>
        </div>
      </div>
    `,
  });
